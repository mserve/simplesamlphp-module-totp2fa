            ...
            if (sspmod_totp2fa_OtpLogicHelper::hasValidLogic($rule) && array_key_exists('settings', $rule) && sspmod_totp2fa_OtpHelper::hasValidSettings($rule['settings'])) {
                ...
                try {
                    if (sspmod_totp2fa_OtpLogicHelper::apply($rule, $request)) {
                        ...
                        SimpleSAML\Logger::info("TOTP2FA CheckAttributes Filter: Rule " . $ruleno . " matches: " . $rule['attributeName'] . " " . $rule['operator'] . " " . $rule['value']);
                        $settings = sspmod_totp2fa_OtpHelper::updateSettings($rule['settings'], $settings);
                    } else {
                        SimpleSAML\Logger::info("TOTP2FA CheckAttributes Filter: Rule " . $ruleno . " fails: " . $rule['attributeName'] . " " . $rule['operator'] . " " . $rule['value']);
                    }
                } catch (\Exception $e) {
                    SimpleSAML\Logger::info("TOTP2FA CheckAttributes Filter: Rule " . $ruleno . " failed with error: " . $e->getMessage());
                }
            }
        }
